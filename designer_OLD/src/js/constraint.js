...
import '../css/constraint.scss'

export default class Constraint {
  constructor (app, id, sourceId, targetId, type) {
    this.app = app
    this.id = id
  ...
  getSourceActivity () {
    return this.app.data.getActivity(this.sourceId)
  }

  getTargetActivity () {
    return this.app.data.getActivity(this.targetId)
  }

  createElement () {
    this.svg.g = this.app.canvas.gConstraints.append('g')
  ...
  createOverviewElement () {
    this.svg.overview = this.app.canvas.overview.gConstraints.append('path')
      .attr('class', 'line')
      .attr('stroke-width', this.size.line)
    return this
  }
  ...
  createTooltip () {
    ...
    template.select('.delete-constraint')
      .on('click', () => {
        ...
        this.app.data.deleteConstraint(this.id)
      })
  ...
  updatePosition (approximatePositioning = false) {
    ...
    let angle = null
    const sourceActivity = this.app.data.getActivity(this.sourceId)
    const targetActivity = this.app.data.getActivity(this.targetId)
