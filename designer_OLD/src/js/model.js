...
/*
*/
export class JSON_Model extends Model {
  ...
  parseString (string) {
    ...
    this.data.activities = json.activities.map(a => new Activity_Model(JSON.stringify(a)))
    this.data.constraints = json.constraints.map(c => new Constraint_Model(JSON.stringify(c)))
  }
}
/*
*/
export class XML_Model extends Model {
...
`
  ...
  parseString (string) {
    const xml = new window.DOMParser().parseFromString(string, 'text\/xml') // eslint-disable-line
    ...
    this.data.activities = Object.values(activities).map(a => {
      const m = new Map()
      Object.keys(a).forEach(k => m.set(k, a[k]))
    ...
    this.data.constraints = Object.values(constraints).map(c => {
      const m = new Map()
      Object.keys(c).forEach(k => m.set(k, c[k]))
  ...
  getJXONData (oXMLParent) {
    ...
    var vResult = /* put here the default value for empty nodes! */ null; var nLength = 0; var sCollectedTxt = ''
    ...
    if (oXMLParent.hasChildNodes()) {
      for (var oItChild, sItKey, sItVal, nChildId = 0; nChildId < oXMLParent.childNodes.length; nChildId++) {
        ...
        if (oItChild.nodeType === 4) { sCollectedTxt += oItChild.nodeValue } /* nodeType is "CDATASection" (4) */ // eslint-disable-line
        else if (oItChild.nodeType === 3) { sCollectedTxt += oItChild.nodeValue.replace(/^\s+|\s+$/g, '') } /* nodeType is "Text" (3) */ // eslint-disable-line
          ...
          if (vResult.hasOwnProperty(sItKey)) { // eslint-disable-line
