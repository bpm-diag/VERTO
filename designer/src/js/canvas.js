...
export default class Canvas {
  constructor (app) {
    ...
    this.svg.on('click', (event) => {
      // const targetId = d3.select(d3.event.target).attr('id')
      const targetId = d3.select(event.target).attr('id')
      if (targetId !== this.svg.attr('id')) return
      this.app.data.deselectAll()
    })
      .on('dblclick', (event) => {
        // const targetId = d3.select(d3.event.target).attr('id')
        const targetId = d3.select(event.target).attr('id')
        if (targetId !== this.svg.attr('id')) return
        // const position = this.fromViewportToSvg(d3.event.offsetX, d3.event.offsetY)
        const position = this.fromViewportToSvg(event.offsetX, event.offsetY)
        this.app.data.createActivity(null, position, true)
      })
      .on('mouseover', () => {
      })
      .on('mouseout', () => {
    ...
    tippy(this.svg.node(), {
      ...
      onShow: () => {
        if (this.app.data.getActivities().length !== 0) return false
      }
    })
    /* overview */
