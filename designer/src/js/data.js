...
export default class Data {
  constructor (app) {
    ...
    d3.select(document).on('keyup.app_data', () => {
      ...
      if (e.code === 'Delete' || e.code === 'MetaRight' || e.code === 'MetaLeft') {
        ...
        this.app.sidepanel.showGlobalMenu()
      }
      // console.log(event)
    })
  }
  ...
  createActivity (id, position, select = false) {
    ...
    this.activities[id] = new Activity(this.app, id, position)
    if (select) this.selectElement(id)
    this.saveModelToCache()
    this.app.sidepanel.updateGlobalMenu()
  ...
  */
  ...
  createConstraint (id, sourceId, targetId, type, select = false) {
    if (id === undefined) id = this.generateID()
    this.constraints[id] = new Constraint(this.app, id, sourceId, targetId, type)
  ...
  selectElement (elementId) {
    ...
    if (element instanceof Activity) this.app.sidepanel.showActivityMenu(element)
    if (element instanceof Constraint) this.app.sidepanel.showConstraintMenu(element)
    return this
  }
  ...
  getXML () {
    ...
    const model = new XML_Model().id(id).name(name).activities(activities).constraints(constraints)
  ...
  getJson () {
    ...
    const activities = this.getActivities()
    const constraints = this.getConstraints()
    const model = new JSON_Model().id(id).name(name).activities(activities).constraints(constraints)
    return model.toString()
  }
