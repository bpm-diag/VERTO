...
export default class Activity {
  ...
  /*
    */
  _createElement () {
    this._svg.g = this.app.canvas.gActivities.append('g')
  ...
  _createOverviewElement () {
    this._svg.overview = this.app.canvas.overview.gActivities.append('rect')
      .attr('id', 'activity-' + this.id)
      .attr('class', 'activity')
      .attr('x', this._svg.x)
      .attr('y', this._svg.y)
  ...
  _createHandles () {
    ...
    this._svg.handles.top.area = this._svg.handles.g.append('rect')
      ...
      .on('mouseover', () => {
        if (this.app.canvas.status.creatingConstraint) return
      ...
      .call(d3.drag()
        ...
        .on('drag', () => this._onDragConstraint(this._svg.handles.top))
    ...
    this._svg.handles.right.area = this._svg.handles.g.append('rect')
      ...
      .call(d3.drag()
        ...
        .on('drag', () => this._onDragConstraint(this._svg.handles.right))
    ...
    this._svg.handles.bottom.area = this._svg.handles.g.append('rect')
      ...
      .call(d3.drag()
        ...
        .on('drag', () => this._onDragConstraint(this._svg.handles.bottom))
    ...
    this._svg.handles.left.area = this._svg.handles.g.append('rect')
      ...
      .call(d3.drag()
        ...
        .on('drag', () => this._onDragConstraint(this._svg.handles.left))
    ...
    /*
        tippy(this._svg.handles.top.area.node(), {
            ...
            onShow: () => {
                if(this.app.data.getConstraints().length != 0) return false;
            }
        });
  ...
  _createTooltip () {
    ...
    template.select('.delete-activity')
      .on('click', () => {
        ...
        this.app.data.deleteActivity(this.id)
      })
  ...
  highlight (bool = true) {
    if (bool) {
      this.app.data.getActivities().forEach(a => {
        if (a.id === this.id) return
        a.partiallyHide(true)
      })
      this.app.data.getConstraints().forEach(c => {
  ...
  _onClick () {
    ...
    this.app.data.selectElement(this.id)
  }
  ...
  _onDrag () {
    this._svg.x += d3.event.x - this._size.width / 2
    this._svg.y += d3.event.y - this._size.height / 2
    this._svg.g.attr('transform', 'translate(' + this._svg.x + ' ' + this._svg.y + ')')
    this._svg.overview.attr('x', this._svg.x).attr('y', this._svg.y)
    this.constraints.forEach(constraintId => this.app.data.getConstraint(constraintId).updatePosition(true))
    return this
  }
  ...
  _onDragEnd () {
    ...
    this.constraints.forEach(constraintId => this.app.data.getConstraint(constraintId).updatePosition(false))
    this.afterUpdate()
    return this
  }

  _onDragStartConstraint (source) {
    this.app.canvas.status.creatingConstraint = true
    this._showDragBorder()
  ...
  _onDragConstraint () {
    this._tempLink.attr('x2', d3.event.x).attr('y2', d3.event.y)
    var p = {
      x: d3.event.x + this._svg.x,
      y: d3.event.y + this._svg.y
    }
    this._tempLinkDestination = undefined
    this.app.data.getActivities().forEach(a => {
      if (a.id === this.id) return
  ...
  _onDragEndConstraint (source) {
    this.app.canvas.status.creatingConstraint = false
    this._showDragBorder(false)
    source.arrow.style('display', 'none')
    ...
    if (this._tempLinkDestination !== undefined) {
      ...
      // const constraint =
      this.app.data.createConstraint(null, this.id, this._tempLinkDestination.id, undefined, true)
      this._tempLink = undefined
      this._tempLinkDestination = undefined
    }
    return this
  ...
  setHeight (w) {
    ...
    this.constraints.forEach(constraintId => this.app.data.getConstraints(constraintId).updatePosition(false))
    return this
  }
  ...
  afterUpdate () {
    this.app.data.saveModelToCache()
  }
}
