...
export default class VERTOConstraintModel {
...
function getAttr (obj, key) {
  ...
  else throw new StrError(`[VERTOConstraintModel] Input obj has not key '${key}'`, obj)
}

function parseFromString (str) {
  if (typeof str !== 'string') throw StrError('[VERTOConstraintModel] Input str in not a string')
  try {
    ...
    console.error(exc)
    throw new StrError('[VERTOConstraintModel] Cannot parser Input str as a json. Invalid format.')
  }
}
