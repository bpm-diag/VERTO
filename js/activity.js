class Activity{
    ...
    */
    _createElement(){
        this._svg.g = app.canvas.gActivities.append("g")
    ...
    _createOverviewElement(){
        this._svg.overview = app.canvas.overview.gActivities.append("rect")
            .attr("id", "activity-" + this.id)
            .attr("class", "activity")
            .attr("x", this._svg.x)
            .attr("y", this._svg.y)
    ...
    */
    _createHandles(){
        ...
        this._svg.handles.top.area = this._svg.handles.g.append("rect")
            ...
            .on("mouseover", () => {
                if(app.canvas.status.creatingConstraint) return;
        ...
        tippy(this._svg.handles.top.area.node(), {
            ...
            onShow: () => {
                if(app.data.getConstraints().length != 0) return false;
            }
        });  
    ...
    _createTooltip(){
        ...
        template.select(".delete-activity")
            .on("click", () => {
                ...
                app.data.deleteActivity(this.id);
            });
    ...
    */
    ...
    _onDrag(){
        ...
        this.constraints.forEach( constraintId => app.data.getConstraint(constraintId).updatePosition(true));
        return this;
    }
    _onDragEnd(){
        if(this._svg.tooltip != undefined) this._svg.tooltip.hide();
        ...
        this.constraints.forEach( constraintId => app.data.getConstraint(constraintId).updatePosition(false));
        this.afterUpdate();
        return this;
    }
    _onDragStartConstraint(source){
        app.canvas.status.creatingConstraint = true;
        this._showDragBorder();
    ...
    _onDragEndConstraint(source){
        app.canvas.status.creatingConstraint = false;
        this._showDragBorder(false);
        source.arrow.style("display", "none");
        ...
        if(this._tempLinkDestination != undefined){
            ...
            let constraint = app.data.createConstraint(null, this.id, this._tempLinkDestination.id, undefined, true);
            this._tempLink = undefined;
            this._tempLinkDestination = undefined;
        }
        return this;
    ...
    */
    setHeight(w){
        ...
        this.constraints.forEach(constraintId => app.data.getConstraints(constraintId).updatePosition(false));
        return this;
    }
    setWidth(w){
