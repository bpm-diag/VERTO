class App_Sidepanel{
    ...
    showActivityMenu(activity){
        ...
        this.activityMenu.select(".delete-activity")
            .on("click", () => {
                app.data.deleteActivity(activity.id);
                this.showGlobalMenu();
            });
    ...
    showConstraintMenu(constraint){
        ...
        this.constraintMenu.select(".delete-constraint")
            .on("click", () => {
                app.data.deleteConstraint(constraint.id);
                this.showGlobalMenu();
            });
        ...
        .on("click", () => {
            app.data.swapConstraintActivities(constraint.id);
        });
        
        this.constraintMenu.select(".body").selectAll("*").remove();
        d3.keys(app.constraintTypes).forEach(groupName => {
            let div = this.constraintMenu.select(".body")
                .append("div").attr("class", "row");
            div.append("div").attr("class", "col-12 constraint-group-name").text(groupName);
            div.selectAll(".constraint-type")
                .data(d3.values(app.constraintTypes[groupName]))
                .enter()
    ...
    updateGlobalMenu(){
        let activities = app.data.getActivities();
        ...
        let activitiesTable = this.globalMenu.select(".tab-content #tab-activities").append("table").attr("class", "table table-sm");
        ...
        activitiesTable.append("tbody")
            ...
            .each(function(d){
                let tr = d3.select(this)
                    ...
                    .on("click", () => app.data.selectElement(d.id));
                tr.append("td").attr("scope", "row").text(d.id);
                tr.append("td").text(d.name);
            });    
        
        let constraints = app.data.getConstraints();
        ...
        let constraintsTable = this.globalMenu.select(".tab-content #tab-constraints").append("table").attr("class", "table table-sm");
